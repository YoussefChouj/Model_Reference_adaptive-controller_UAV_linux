...
chunks = json.loads(Path('.graphify_chunks.json').read_text())
...
DEEP_MODE=true: be aggressive with INFERRED edges - indirect deps, shared assumptions, latent couplings. Mark uncertain ones AMBIGUOUS instead of omitting.
...
Path('.graphify_prompts.json').write_text(json.dumps(prompts))
print(f'Built {len(prompts)} prompts')
print(f'Avg prompt length: {sum(len(p) for p in prompts) // len(prompts)} chars')