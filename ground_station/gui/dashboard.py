...
class Dashboard:
    ...
    def _read_selected_port_and_baud(self) -> Tuple[str, int]:
        ...
        # Dropdown current value will override.
    ...
    def _frame(self) -> None:
        ...
        try:
            dpg.configure_item("vrc_sdk_btn", enabled=True)
        except Exception:
            pass
        ...
        try:
            ...
            fly_txt = "FlyMode: SDK" if fm >= 0.5 else "FlyMode: RC"
        ...
        try:
            ...
            dpg.set_value(
                "mon_footer",
                f"sbus_lost={sbus_txt} | ARM={'ON' if arm_on else 'OFF'} | {fly_txt} | bench={'ON' if self._bench_mode_ui else 'OFF'}",
            )
        except Exception:
            pass
    ...
    def _ensure_vofa_executable(self) -> Optional[str]:
        ...
        if isinstance(configured, str) and configured and Path(configured).exists():
            return configured
        exe_in_path = shutil.which("vofa+")
        if exe_in_path:
            return exe_in_path
        return None

    def _open_plot(self, workspace_rel: str) -> None:
        # workspace_rel like "presets/vofa/mrac_errors.vofa"
        vofa = self._ensure_vofa_executable()
        if vofa is None:
            self._show_vofa_path_dialog()
            return
        workspace_path = (self.repo_root / workspace_rel).resolve()
        try:
            subprocess.Popen([vofa, "--workspace", str(workspace_path)], cwd=str(self.repo_root))
    ...
    def _on_bench_toggle(self, sender: Any, app_data: Any, user_data: Any) -> None:
        ...
        try:
            ...
            if not self._bench_mode_ui:
                t = float(dpg.get_value("vrc_thr_slider"))
                if t > RC_THR_MAX:
                    dpg.set_value("vrc_thr_slider", float(RC_THR_MAX))
        except Exception:
            pass
        try:
    ...
    def _build_gui(self) -> None:
        ...
        ports: List[str] = [default_port]
        try:
            import serial  # type: ignore

            ports = [p.device for p in serial.tools.list_ports.comports()] or [default_port]
        ...
        with dpg.window(
            ...
            with dpg.handler_registry(tag="hk_global_space"):
                dpg.add_key_press_handler(dpg.mvKey_Space, callback=lambda s, a: self._emergency_stop())
            dpg.bind_item_handler_registry("main_window", "hk_global_space")
            ...
            with dpg.group(horizontal=True):
                ...
                with dpg.child_window(width=200, height=-1, border=True):
                    ...
                    dpg.add_button(
                        label="MRAC Errors",
                        width=-1,
                        callback=lambda: self._open_plot("presets/vofa/mrac_errors.vofa"),
                    )
                    dpg.add_button(
                        label="Adaptive Weights",
                        width=-1,
                        callback=lambda: self._open_plot("presets/vofa/weights.vofa"),
                    )
                    dpg.add_button(
                        label="PID Loops",
                        width=-1,
                        callback=lambda: self._open_plot("presets/vofa/pid_all.vofa"),
                    )
                    dpg.add_button(
                        label="Full Telemetry",
                        width=-1,
                        callback=lambda: self._open_plot("presets/vofa/full.vofa"),
                    )
