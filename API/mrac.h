...
// ---------------------------------------------------------------------------
// Per-Component Projection Limits (What_limit[]) and Tolerances (What_tol[])
// ---------------------------------------------------------------------------
// Formula:  W_lim[i] = max_disturbance_torque_i / theta_max_i
// Tolerance = 20% of limit (smooth projection zone)
//
// Physical interpretation — PITCH / ROLL (UA_MAX = 3.369 Nm):
//   bias  (theta=1.0)   : W_lim=0.50 → 0.50 Nm  constant rejection
//                         handles CG offset 14mm, suspension spring bias
//   angle (theta≤0.524) : W_lim=1.00 → 0.524 Nm at 30°
//                         compensates spring stiffness k_s up to 1.0 Nm/rad
//                         handles ΔJ inertia error up to ~430%
//   rate  (theta≤1.047) : W_lim=0.10 → 0.105 Nm
//                         compensates damping coefficient error ΔB≤0.10 Nm/(rad/s)
//   drag  (theta≤1.047) : W_lim=0.05 → 0.052 Nm   nonlinear drag error
//   un    (LOE frac, ≤1): W_lim=0.80 → up to 80% Loss-of-Effectiveness recovery
//                         What_un=-0.3 ≡ 30% LOE detected, ua adds back 30%*un
//   v     (perf.rec.)   : W_lim=0.20 → performance recovery ≤20% of v
//
// Physical interpretation — YAW (UA_MAX = 2.027 Nm):
//   bias:  0.30 Nm  (motor CW/CCW asymmetry, yaw spring bias)
//   angle: 0.50 Nm at yaw max
//   rate:  0.052 Nm damping error
//   drag:  0.021 Nm
//
// Max |ua| per case (pitch/roll):
//   Case 1 Struct 4B:    0.50+0.52+0.10+0.05 = 1.17 Nm  (pure disturbance rejection)
//   Case 2 Struct+un+v:  1.17 + 0.80*un + 0.20*v        (up to 3.1 Nm with LOE)
//   Case 3 RBF 6B:       6*0.56*1.0 = 3.37 Nm worst-case (all RBFs fire)
//   Case 4 RBF+un+v:     6*0.34 + LOE + perf ≈ 3.6 Nm
// ---------------------------------------------------------------------------

#if USE_STRUCTURED_UNCERTAINTY == 1
  #if INCLUDE_CONTROL_IN_REGRESSOR == 1
    //  Case 2: basis = [bias, angle, rate, drag, un, v]  — 6 components 
    //  gamma[i] ∝ 1/typical_theta_i²: compensates for regressor magnitude imbalance   
    //  gamma_un=0.10 (slow LOE learning, ~30s), gamma_v=1.00 (faster perf recovery)   
    #define PITCH_GAMMA_INIT  {0.50f, 3.30f, 1.00f, 2.00f, 0.10f, 1.00f}
    #define ROLL_GAMMA_INIT   {0.50f, 3.30f, 1.00f, 2.00f, 0.10f, 1.00f}
    #define YAW_GAMMA_INIT    {0.30f, 2.00f, 0.70f, 1.50f, 0.10f, 1.00f}
    #define PITCH_WLIM_INIT   {0.50f, 0.60f, 0.40f, 0.10f, 0.40f, 0.20f}
    #define ROLL_WLIM_INIT    {0.50f, 0.60f, 0.40f, 0.10f, 0.40f, 0.20f}
    #define YAW_WLIM_INIT     {0.30f, 0.40f, 0.20f, 0.05f, 0.30f, 0.20f}
    #define PITCH_WTOL_INIT   {0.10f, 0.12f, 0.08f, 0.02f, 0.08f, 0.04f}
    #define ROLL_WTOL_INIT    {0.10f, 0.12f, 0.08f, 0.02f, 0.08f, 0.04f}
    #define YAW_WTOL_INIT     {0.06f, 0.08f, 0.04f, 0.01f, 0.06f, 0.04f}
  #else
    //  Case 1: basis = [bias, angle, rate, drag]  — 4 components               
    #define PITCH_GAMMA_INIT  {0.50f, 3.30f, 1.00f, 2.00f}
    #define ROLL_GAMMA_INIT   {0.50f, 3.30f, 1.00f, 2.00f}
    #define YAW_GAMMA_INIT    {0.30f, 2.00f, 0.70f, 1.50f}
    #define PITCH_WLIM_INIT   {0.50f, 1.00f, 0.10f, 0.05f}
    #define ROLL_WLIM_INIT    {0.50f, 1.00f, 0.10f, 0.05f}
    #define YAW_WLIM_INIT     {0.30f, 0.50f, 0.05f, 0.02f}
    #define PITCH_WTOL_INIT   {0.10f, 0.20f, 0.02f, 0.01f}
    #define ROLL_WTOL_INIT    {0.10f, 0.20f, 0.02f, 0.01f}
    #define YAW_WTOL_INIT     {0.06f, 0.10f, 0.01f, 0.00f}
  #endif
#else //  RBF 
  #if INCLUDE_CONTROL_IN_REGRESSOR == 1
    //  Case 4: basis = [rbf_ang×3, rbf_rate×3, un, v]  — 8 components          
    //  gamma for RBF: phi∈[0,1], typical phi²≈0.25 → gamma_rbf = 0.50/0.25=2.0 
    #define PITCH_GAMMA_INIT  {2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 0.10f, 1.00f}
    #define ROLL_GAMMA_INIT   {2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 0.10f, 1.00f}
    #define YAW_GAMMA_INIT    {2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 0.10f, 1.00f}
    //  W_lim per RBF: 0.60*UA_MAX/6 = 0.60*3.369/6 = 0.337 (pitch/roll)       
    //  Remaining 0.40*UA_MAX reserved for un (+LOE) and v (perf recovery)       
    #define PITCH_WLIM_INIT   {0.34f, 0.34f, 0.34f, 0.34f, 0.34f, 0.34f, 0.80f, 0.20f}
    #define ROLL_WLIM_INIT    {0.34f, 0.34f, 0.34f, 0.34f, 0.34f, 0.34f, 0.80f, 0.20f}
    #define YAW_WLIM_INIT     {0.20f, 0.20f, 0.20f, 0.20f, 0.20f, 0.20f, 0.80f, 0.20f}
    #define PITCH_WTOL_INIT   {0.07f, 0.07f, 0.07f, 0.07f, 0.07f, 0.07f, 0.16f, 0.04f}
    #define ROLL_WTOL_INIT    {0.07f, 0.07f, 0.07f, 0.07f, 0.07f, 0.07f, 0.16f, 0.04f}
    #define YAW_WTOL_INIT     {0.04f, 0.04f, 0.04f, 0.04f, 0.04f, 0.04f, 0.16f, 0.04f}
  #else
    //  Case 3: basis = [rbf_ang×3, rbf_rate×3]  — 6 components                 
    //  W_lim per RBF: UA_MAX/N = 3.369/6 = 0.562 (pitch/roll), 2.027/6=0.338 (yaw) 
    #define PITCH_GAMMA_INIT  {2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 2.00f}
    #define ROLL_GAMMA_INIT   {2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 2.00f}
    #define YAW_GAMMA_INIT    {2.00f, 2.00f, 2.00f, 2.00f, 2.00f, 2.00f}
    #define PITCH_WLIM_INIT   {0.56f, 0.56f, 0.56f, 0.56f, 0.56f, 0.56f}
    #define ROLL_WLIM_INIT    {0.56f, 0.56f, 0.56f, 0.56f, 0.56f, 0.56f}
    #define YAW_WLIM_INIT     {0.34f, 0.34f, 0.34f, 0.34f, 0.34f, 0.34f}
    #define PITCH_WTOL_INIT   {0.11f, 0.11f, 0.11f, 0.11f, 0.11f, 0.11f}
    #define ROLL_WTOL_INIT    {0.11f, 0.11f, 0.11f, 0.11f, 0.11f, 0.11f}
    #define YAW_WTOL_INIT     {0.07f, 0.07f, 0.07f, 0.07f, 0.07f, 0.07f}
  #endif
#endif
