"""Gazebo transport bridge for the digital twin (spec 4b).
...
subprocess, loads the JX_FLY URDF generated by :mod:`sim.urdf`, and acts
as the bridge between the controller's per-axis mixer outputs and the
Gazebo world.
...
- ``/world/jx_fly/motor_command`` : ``gz.msgs.Actuators`` (4 channels,
  per-motor thrust in newtons). The custom system plugin or a
  ``gz::msgs::Actuators`` -> ``Force`` adapter translates this into
  per-motor forces.

Subscribers (gz sim -> bridge):

- ``/world/jx_fly/pose`` : ``gz.msgs.Pose`` (current inertial pose + twist).
...
class GazeboBridge:
    ...
    # The custom system plugin adds our specific channel names.
    TOPIC_MOTOR = "/world/jx_fly/motor_command"
    ...
    def _setup_transport(self) -> None:
        ...
        """
        # Publisher: motor commands. gz::msgs::Actuators carries N
        # channel values (we use 4). The custom plugin reads them.
        self._motor_pub = self._node.advertise(
            self.TOPIC_MOTOR, self._gz["actuators"].Actuators)
        if not self._motor_pub:
            raise GazeboBridgeError(
                f"failed to advertise on {self.TOPIC_MOTOR}; "
                f"check the SDF for the JX_FLY model plugin."
            )

        # Subscriber: pose. The custom plugin publishes a Pose msg with
        # both inertial position/orientation and body-frame velocity.
        def on_pose(msg):
            with self._pose_lock:
                # Convert gz::msgs::Pose into our BridgeState fields.
                # Note: the exact field names depend on the SDF plugin;
                # in the canonical plugin we ship, they are msg.position
                # and msg.orientation for the pose, and the linear/angular
                # velocity is on a separate twisty field. We read whichever
                # is present and leave the rest at default.
                p = msg.position
                o = msg.orientation
                self._latest_pose = {
                    "x": p.x, "y": p.y, "z": p.z,
    ...
    def send_motor_command(self, motor_thrusts_N: np.ndarray) -> None:
        """Send a per-motor thrust (N) to the sim. shape (4,)."""
        motor_thrusts_N = np.asarray(motor_thrusts_N, dtype=float).reshape(4)
        msg = self._gz["actuators"].Actuators()
        for i, f in enumerate(motor_thrusts_N):
            msg.position.append(0.0)
            msg.velocity.append(0.0)
            msg.normal.append(float(f))
    ...
    def step(self, motor_thrusts_N: np.ndarray, dt: float) -> BridgeState:
        ...
        """
        self.send_motor_command(motor_thrusts_N)
    ...
    def reset(self) -> None:
        ...
        # service that the canonical plugin implements. For now we
        # re-initialise the local state cache so the next step() reads
        # from a known baseline.
        self._prev_z = None
        # Service request goes here. Stub for now -- the canonical
        # plugin implements world/reset, but the bridge is also valid
        # without it (the sim resumes its deterministic initial state
        # the next time the controller calls send_motor_command).
