...
class ChannelsSingleSourceOfTruth(unittest.TestCase):
    ...
    def test_grab_poller_uses_channels(self):
        ...
        import channels
        import grab_poller
        # grab_poller.WATCH should be a list of IDs that all exist in CHANNELS
...
class FormatPaperGuard(unittest.TestCase):
    ...
    def test_long_abstract_truncated_or_split(self):
        ...
        out = digest.format_paper(p)
        # Either truncated to 2000, or split into 2 messages. Both are valid.
        if isinstance(out, list):
            for chunk in out:
                self.assertLessEqual(len(chunk), 2000,
                                     f"chunk exceeds 2000: {len(chunk)}")
            # combined: original info still in there somewhere
            joined = "\n".join(out)
...
class CrashReporter(unittest.TestCase):
    ...
    def test_crash_posts_to_briefing_channel(self):
        # Reload digest and break it: make load_all_priorities raise.
        # Then call main() and assert the crash post happens.
        import digest
        import channels
        ...
        # content should mention crash/error
